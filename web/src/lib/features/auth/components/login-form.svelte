<script lang="ts">
	...
	import { FieldDescription, FieldGroup, FieldSeparator } from '$lib/components/ui/field/index.js';
	...
	const form = superForm(data, { validators: zod4Client(loginSchema) });
	const { form: formData, errors, enhance, submitting } = form;

	// A failed login is form-level, never per-field — see error-map.ts. Rendering it under an input
		...
		<Card.Content>
			...
			<form method="POST" use:enhance>
				<FieldGroup>
					...
					<!-- Not wired to anything: there is no OAuth on the API. Kept as designed. -->
